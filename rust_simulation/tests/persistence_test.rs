...
#[test]
fn test_saving_with_no_players_clears_stale_data() {
    let test_file = "q_tables.json";

    // Run the test in a closure to ensure cleanup happens even on panic.
    let result = panic::catch_unwind(|| {
        // 1. Setup
        // Create a stale save file.
        fs::write(test_file, "{\"stale\":\"data\"}").expect("Failed to create stale save file");
        ...
        // No players are added to the world.
        ...
        // 3. Verify the output file
        let content = fs::read_to_string(test_file).expect("Failed to read test file");
        // The file should now contain an empty JSON object.
