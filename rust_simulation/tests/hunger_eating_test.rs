...
#[test]
fn test_eating_non_food_removes_intent() {
    // 1. Setup
    let mut app = setup_test_app();
    let mut inventory = Inventory::new();
    inventory.add_item("wood", 1); // Wood is not food
    ...
    // Hunger should not change (minus the normal hunger rate)
    let config = app.world.resource::<Config>();
    assert_eq!(hunger.current, 50.0 - config.survival.hunger_rate);
    // Wood should still be in inventory
    assert_eq!(inventory.get_quantity("wood"), 1);
    // The intent to eat should be removed
    assert!(app.world.get::<WantsToEat>(entity).is_none());
}
