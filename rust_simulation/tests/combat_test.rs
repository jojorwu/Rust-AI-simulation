...
#[test]
fn test_combat_system_clamps_health_at_zero() {
    // 1. Setup
    let mut app = App::new();
    app.add_plugins(MinimalPlugins);
    app.add_event::<Event>();
    ...
    // Create target with 5 health
    let target = app.world.spawn(Health { current: 5, max: 50 }).id();
    // Create attacker dealing 10 damage
    let _attacker = app
        .world
        .spawn((WantsToAttack { target }, Damage(10)))
        .id();
    ...
    // 3. Verify
    // Target's health should be exactly 0, not -5
    let target_health = app
        .world
        .get::<Health>(target)
        .expect("Target should have a Health component");
    ...
    // A death event should still be sent
    let events = app.world.resource::<Events<Event>>();
    let mut reader = events.get_reader();
    let mut death_event_found = false;
    for event in reader.read(events) {
