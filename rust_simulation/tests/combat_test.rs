...
#[test]
fn test_combat_system_handles_overkill_and_multiple_attackers() {
    // 1. Setup
    let mut app = App::new();
    app.add_plugins(MinimalPlugins);
    app.add_event::<Event>();
    ...
    // Create target with 10 HP
    let target = app.world.spawn(Health { current: 10, max: 10 }).id();

    // Create two attackers, each dealing 10 damage
    ...
    // 3. Verify
    // Target's health should be exactly 0, not negative.
    let target_health = app
        .world
        .get::<Health>(target)
        .expect("Target should have a Health component");
    assert_eq!(target_health.current, 0);

    // Exactly one death event should have been sent
    let events = app.world.resource::<Events<Event>>();
    let mut reader = events.get_reader();
