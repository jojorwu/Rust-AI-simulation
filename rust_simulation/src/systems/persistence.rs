use crate::{components::ai::GoalQTable, player::Player, AppPaths};
...
fn save_q_tables(
    ...
    let final_path = app_paths.data_dir.join("q_tables.json");
    let temp_path = app_paths.data_dir.join("q_tables.json.tmp");
...
pub fn save_q_tables_on_exit(query: Query<(&Player, &GoalQTable)>, app_paths: Res<AppPaths>) {
    info!("Saving Q-tables...");
    if let Err(e) = save_q_tables(&query, &app_paths) {
        error!("Failed to save Q-tables: {e}");
    } else {
        info!("Q-tables saved successfully to {:?}", app_paths.data_dir);
    }
}
